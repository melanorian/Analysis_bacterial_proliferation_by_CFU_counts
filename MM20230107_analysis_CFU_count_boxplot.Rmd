...
```{r}
pst_raw <- read.delim("C:/Users/Mende012/Documents/Bioinformatics/CFU_counts/MM20230113_summary_CFU_count_calculations.txt")
head(pst_raw)
...
# 2.1 Posthoc to see which groups are significantly different (TUKEY) 
# 2.2 generate labels
...
pdf(file = "C:/Users/Mende012/Documents/Bioinformatics/CFU_counts/MM20220606_CFU_count_raw_st.pdf",
     width = 8,
     height = 6)
...
################################################################################
#######################  SCALING BY POSITIVE CONTROL DC30000 ###################
################################################################################

# see if scaling by positive control changes results
```{r}
...
# x <- "16-3-2022"
#positive_control <- "DC3000"
# rm(ls=date)

# creat/test function
#med_pos <- function(df, date, positive_control){
#  a <- as.data.frame(simple_df_norm[simple_df_norm$Date == date,]) 
#  b <- as.data.frame(a[a$Pst_strain_n == positive_control,])
#  contr_median <- median(b$Pst_CFU_n) # change here from median to median
#  return(contr_median)
#}
...
#mult <- function(date){
#  rows <- nrow(simple_df_norm[simple_df_norm$Date == date,]) # count rows for of each assay
#  a <- df_ctrmed[df_ctrmed$Date == date,] # access the median/median of a respective control
#  re_assay_date <- unlist(rep(a$Date, times = rows)) # times assayID as many times as there are rows for the assayID
...
#  b <- data.frame(unlist(re_assay_date), unlist(re_median)) # make a df of length = assay rows
#  return(b)
#}
...
# export 
write.csv(df_median, "./Median_CFU_counts_scaled.csv")
```
...
# close pdf
ggsave("MM20230107_CFU_RLU_simple_scaled.svg",width = 7, height = 4)
g4
dev.off()
